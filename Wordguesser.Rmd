...
#Small list of more normal words

words_norm <- read.table("words2.txt")
colnames(words_norm) <- "Word"
words_norm$Word <- tolower(words_norm$Word)

```

Starting Words guide
```{r}
Starting_Word <- c('OATER/ORATE/ROATE', 	'REALO', 'ALERT/ALTER/LATER', 'IRATE/RETIA/TERAI', 'ARIEL/RAILE', 
                   'AEROS/AROSE/SOARE', 'ASTER/STARE/TEARS', 'EARLS/LASER', 'ARISE/RAISE', 'ANTRE/EARNT')

Avg_correct_letters <- c(1.789, 1.781, 1.778, 1.778, 1.770, 1.768, 1.765, 1.757, 1.757, 1.736)

starting_word_df <- data.frame('Starting Word' = Starting_Word,  'Avg correct letters' = Avg_correct_letters)
...
join_three_df <- function(df1 = NULL, df2 = NULL, df3 = NULL) {
  ...
  if (is.null(df1) & is.null(df2)) {
    ...
    result <-  df1 %>% merge(df2, by = "Word") %>% merge(df3, by = "Word")
  }
...
max_occur_let <- function(df, let_index) {
  #colnames(df) <- c("Word", n)
  result <- df %>% 
    mutate(letter = substring(Word, let_index, let_index)) %>%
    count(letter) %>%
    arrange(desc(n))
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
    textInput(inputId = "good_letters", label = "Included letters:", value = "",
              placeholder = "Included letters"),
    textInput(inputId = "bad_letters", label = "Excluded letters:", value = "", 
    ...
    strong("Input letters by position:"),
      ...
      strong("Included letters out of position:"),
      splitLayout(
      ...
      strong("Common Letter index:"),
      numericInput("index", "", value = 1, 
                   width = 100, min = 1, max = 5),
      textOutput("Most common letter"),
          verbatimTextOutput("maxLet")
    ), 
    
    mainPanel(
        tabsetPanel(
          tabPanel("Starting Words", tableOutput("startingWords")),
          tabPanel("Possible Words", tableOutput("allWords")),
          tabPanel("Common Words", tableOutput("normWords")),
          tabPanel("Letter Freq", tableOutput("common"))
...
server <- function(input, output, session) {
  ...
  results <- reactive({
    join_three_df(
      join_six_df(good_bad_letters(input$bad_letters, FALSE, words_df)
                  , good_bad_letters(input$good_letters, TRUE, words_df)
  ...
  final_results <- reactive({
    toString(results()$Word)
  })
  
  
  max_let <- reactive({
    max_occur_let(results()["Word"], input$index) #column name is changing here
  })
  
  norm_words <- reactive({
    toString(join_three_df(results(), words_norm_df)$Word)
  })
  
  
  output$common <- renderTable({max_let()})
  output$allWords <- renderTable({final_results()})
  output$normWords <- renderTable({norm_words()})
  output$startingWords <- renderTable(starting_word_df)
